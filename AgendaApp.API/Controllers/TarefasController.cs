    ...
    public class TarefasController : ControllerBase
    {
        ...
        public IActionResult Put(Guid id, TarefaRequestDto request)
        {
            try
            {
                ...
                tarefa.Categoria = null; //apagando o vinculo com a categoria
            ...
            public IActionResult Delete(Guid id)
            {
                try
                {
                    //consultando uma tarefa no banco de dados através do ID
                    var tarefaRepository = new TarefaRepository();
                    var tarefa = tarefaRepository.GetById(id);

                    //verificando se o registro não foi encontrado no banco de dados
                    if (tarefa == null)
                        //HTTP 404 (NOT FOUND)
                    ...
                    //HTTP 200 (OK)
                    return StatusCode(200, new { mensagem = "Tarefa excluida com sucesso." });
                }
                catch (Exception e)
                {
                    //HTTP 500 (INTERNAL SERVER ERROR)
            ...
            public IActionResult GetById(Guid id)
            {
                try
                {
                    ...
                    //copiando os dados obtidos na consulta para classe DTO
